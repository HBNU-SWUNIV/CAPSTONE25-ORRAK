# 한밭대학교 컴퓨터공학과 ORRAK
**초보 클라이머를 위한 자세 추정 기반 분석 도구**

**팀 구성**
- 20201047 구남석

## <u>Teamate</u> Project Background
- **분석 도구의 필요성**
  - 초보 클라이머는 지도 없이 자신의 자세가 올바른지 판단하기 어렵고, 잘못된 자세로 인해 부상 위험이 높다.
  - 단순하게 촬영된 영상으로는 무게 중심, 삼지점 유지 여부를 확인할 수 없다.
  - 따라서, **사용자가 스스로 자세 분석 영상을 보고 올바른 자세로 완등할 수 있도록 지원하는 도구**가 필요하다.

- **기존 해결책의 문제점**
  - 기존 전문가 코칭은 비용과 시간 측면에서 접근성이 낮아, 초보자가 지속적으로 받기 어렵다.
  - 단순 영상 피드백이나 지시 중심의 코칭은 사용자가 **왜 해당 자세를 취해야 하는지**, **무게 중심과 삼지점 유지가 갖는 의미**를 직관적으로 이해하기 어렵다.  
    따라서, 사용자가 영상을 보고 스스로 루트 파인딩과 자세 교정 능력을 기르는 데 한계가 있다.
  
## System Design
- **녹화 영상 분석**: 사용자가 벽을 올라가는 동안 촬영된 영상을 기반으로, 관절 좌표와 무게 중심을 계산한다.
- **직관적 피드백 UI**: 영상 위에 관절 위치, 안정/불안정 상태, 무게 중심점 위치, 삼지점 유지 여부를 색상과 표시로 시각화한다.  

## Analysis Process Description
<img src="Images/sample5.png" width="500" height="500">

### 무게 중심(COS, Center Of Mass) 계산
- Mediapipe 기반 포즈 추정으로 주요 관절 좌표 33개를 추출한다.
- Mediapipe로 추출된 Landmark(x,y,z) 좌표에 대하여 인간의 각 신체 세그먼트(상완, 전완, 대퇴, 경골 등)의 질량 비율을 고려하여 **세그먼트 중심 좌표**를 구한다.
- 전체 무게 중심은 각 세그먼트 중심과 질량을 가중 평균하여 계산한다:

$\[
\text{COM} = \frac{\sum_i (\text{Segment COM}_i \times \text{mass ratio}_i)}{\sum_i \text{mass ratio}_i}
\]$

### 기저면(BOS, Base of Support) 계산

- **정의**  
  기저면(Base of Support, BOS)은 클라이머의 신체가 벽과 접촉하는 지점(손, 발 등)이 만드는 **지지 다각형 영역**이다.  
  이 영역은 클라이머가 안정적으로 자세를 유지할 수 있는 공간을 의미하며,  
  무게 중심(COM)이 이 영역 안에 존재할 때 자세가 안정적이라 판단한다.

- **BOS 계산 과정**
1. Mediapipe로 검출된 신체 관절 중 `left_index`, `right_index`, `right_foot_index`, `left_foot_index` 좌표를 추출한다.  
2. 현재 프레임에서 벽과 접촉 중인 손·발을 식별하고, 4개 지점의 각각 (x, y, z) 좌표를 선택한다.
3. 추출한 네 점의 좌표는 0.0~1.0 사이의 비율 값으로, 이를 현재 영상의 해상도(width, height)에 맞춰 실제 픽셀 좌표로 변환한다.
4. 변환된 좌표를 이어서 **삼각형(또는 사각형) 기저면**을 생성한다.  
5. 무게 중심 계산 알고리즘으로 추정된 노란색 점이 기저면 내 위치하면 초록색으로, 기저면 바깥에 위치하면 빨간색으로 표시하여 안정 상태를 구분한다  

### 관절 각도 계산
- 사용자의 팔꿈치, 어깨, 손목 등 주요 관절 좌표를 Mediapipe로 추출한다.
- 관절 각도는 벡터 연산으로 계산하며, 예를 들어 팔꿈치 각도는 아래와 같이 구한다:

$\[
\text{Elbow Angle} = \arccos \left( \frac{(A-B) \cdot (C-B)}{\|A-B\| \|C-B\|} \right)
\]$

- 여기서 \(A, B, C\)는 각각 상완, 팔꿈를 통해 **삼지점 유지 및 안정성**을 직관적으로 확인할 수 있으며, 올바른 자세로 완등할 수 있도록 한다.
  
## Conclusion
- 본 분석 도구는 초보 클라이머가 지도 없이도 자신의 자세를 객관적으로 평가하고 개선할 수 있는 피드백 도구를 제공한다.
- 관절 각도 계산과 무게 중심(COM) 분석을 통해 **=삼지점 유지 및 안정성**을 직관적으로 확인할 수 있으며, 올바른 자세로 완등할 수 있도록 한다.
---
## UI 
<img src="Images/UI1.jpg" width="400" height="650">
<img src="Images/UI2.jpg" width="400" height="650">

## 사용자 시나리오
1. [초보 클라이머를 위한 자세 추정 기반 분석 도구](https://huggingface.co/spaces/sirius-koo/ORRAK-INFO) 서버에 접속한다.
2. 사용자는 자신의 디바이스에서 영상을 업로드한다.
3. 서버로부터 분석 영상을 기다린다.
4. 분석된 영상을 바로 시청 또는 다운로드한다.
5. 자세 분석 피드백 예시를 참고하여, 사용자의 자세를 확인한다.

| <img src="Images/sample2.jpg" width="500" height="500"> | <img src="Images/sample3.jpg" width="500" height="500"> |
| :---: | :---: |
| 안정 상태 | 불안정 상태 |
- 왼쪽 사진과 같이 무게 중심(COM)이 기저면(BOS) 내부에 위치할 경우 녹색으로, 오른쪽처럼 COM이 기저면 밖으로 벗어나면 빨간색으로 시각화된다.  
- 상태를 시각적으로 표현하는 UI를 제공할 뿐, 자세 교정 피드백은 사용자가 직접 수행한다.
  예를 들어, 사용자는 불안정 상태 영상을 보고 “왼발을 벽에 두어 삼지점을 만들면 더 안정적이겠다”는 판단을 내리며 자세를 교정할 수 있다.
